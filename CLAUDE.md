...
- Kotlin 2.3.10 / Java 11 (Android minSdk 28, compileSdk 36) + Jetpack Compose for TV (tv-material 1.0.0), Compose BOM 2025.05.01, lifecycle-viewmodel-compose, Hilt 2.59.1, hilt-navigation-compose 1.2.0, Media3 ExoPlayer 1.9.2, Coil 3.4.0, ZXing 4.3.0, kotlinx-coroutines 1.9.0 (007-song-list-ui)
- In-memory only — SongLibrary (session-scoped), Session (session-scoped), MedleyPlaylist (screen-visit-scoped) (007-song-list-ui)
...
## Recent Changes
- 007-song-list-ui: Added Kotlin 2.3.10 / Java 11 (Android minSdk 28, compileSdk 36) + Jetpack Compose for TV (tv-material 1.0.0), Compose BOM 2025.05.01, lifecycle-viewmodel-compose, Hilt 2.59.1, hilt-navigation-compose 1.2.0, Media3 ExoPlayer 1.9.2, Coil 3.4.0, ZXing 4.3.0, kotlinx-coroutines 1.9.0
...
## TOOLS
**IMPORTANT:** Avoid full file reads, prefer using symbol saerch. Prefer tools like `workspaceSymbol` , `incomingCalls`, `findReferences` and `goToDefinition` over `Glob`, `Search`.
**NEVER** use bash grep only as a last resort.

##AGENTS
- BEFORE every task consider: Is the task larger than the 1.5*prompt length for the fix? If so -> subagents do it. Else you do it
...
- Run `./gradlew build` for project-wide verification

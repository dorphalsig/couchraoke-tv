...
- [X] T031 [P] [US1] Write `QrPayloadEncoderTest` in `«test»/presentation/qr/QrPayloadEncoderTest.kt`: the payload is exactly `ws://<ip>:<port>/?token=<CODE>` and carries no discovery-service identifier (FR-007).
- [X] T032 [P] [US1] Write `JoinViewModelTest` in `«test»/presentation/join/JoinViewModelTest.kt`: `connectedCount` tracks the coordinator's connected list and follows a join with no user action (SC-007), and the view model performs no I/O (FR-035).
...
- [X] T036 [US1] Wire session start in `«main»/di/SessionComponent.kt` and `«main»/MainActivity.kt`: resolve the active IPv4, acquire the multicast lease, start the transport, publish the announcement with instance name `KaraokeTV-<noun>`, and verify the registered name matches (FR-004, FR-005, FR-008). **Also implement the production `ControlConnectionHandler`** — the join dispatch that reads the peer's first frame, decodes it with `ControlMessageCodec`, calls `authorize(token)` then `admit(hello)`, sends the `sessionState` reply on acceptance, and routes the socket close to `onDisconnected`. No task previously owned this (see spec.md Observation 18): T016 declared the interface and T018 made `KtorControlTransport` accept one, but nothing implemented it, so the transport had no way to reach the coordinator and T037 could not pass. Refusal branches follow `authorize`/`admit`, which still `TODO()` for everything except the accept path until T050 — do not implement refusal policy here, only the dispatch that will carry it.
- [X] T037 [US1] Add the happy-path case to `«test»/gate/LoopbackJoinGateTest.kt`: `--join-only` with the correct token exits 0, returns `sessionState`, and yields `connectionId` ≥ 1; a second peer with a different `--client-id` also joins and receives a **distinct** `connectionId` (acceptance scenario 4). *(Also carries a wrong-token case — `--join-only` with a token that is not the displayed code must exit 3 with `invalid_token` and close 1008. Without it the happy path proves nothing, since a handler that accepts every token passes it too; that was the actual first implementation. See spec.md Observation 19.)*
- [X] T038 [P] [US1] Implement `QrPayloadEncoder` in `«main»/presentation/qr/QrPayloadEncoder.kt` — pure string construction, no ZXing type in the signature so it stays coverage-selected.
- [X] T039 [P] [US1] Implement `QrBitmapRenderer` and the `QrCode` composable in `«main»/presentation/qr/`: call `QRCodeWriter.encode` with `EncodeHintType.MARGIN = 4` for the four-module quiet zone, and draw the `BitMatrix` into a Compose `ImageBitmap`, dark-on-light and static (FR-030).
- [X] T040 [US1] Implement `JoinUiState` and `JoinViewModel` in `«main»/presentation/join/`: expose the join code, the QR payload, and `connectedCount` derived from `coordinator.connectedDevices.size` — the live connections, never the roster size (FR-025, SC-007).
- [X] T041 [US1] Implement `JoinOverlay` in `«main»/presentation/join/JoinOverlay.kt` as a modal over the song-list shell, QR dominant with the join code directly beneath, connected count shown, entrance limited to a single short fade or scale-fade, and nothing intruding into the QR or its quiet zone (FR-030, FR-031, FR-032). Dismissing returns to the shell without ending the session or disconnecting anyone (FR-033). Tag nodes with test tags so T061 can assert their bounds.
...
- [X] T044 [P] [US2] Write `JoinCodeMatcherTest` in `«test»/domain/control/JoinCodeMatcherTest.kt`: `SWIFT-PANDA`, `swift-panda` and `swiftpanda` all match; null, blank and a wrong code do not. *(Landed with T048 under T036 — `JoinCodeMatcherTest` already covers every listed case: `SWIFT-PANDA`, `swift-panda` and `SwiftPanda` match, and null, empty, whitespace-only, a different adjective, a different noun and the two words reversed do not. See spec.md Observation 19.)*
- [X] T045 [P] [US2] Extend `«test»/domain/session/SessionRosterTest.kt` for capacity: the 11th previously-unseen device is `AtCapacity`, and capacity is evaluated only for unseen devices.
- [X] T046 [P] [US2] Add a virtual-time deadline test to `«test»/domain/session/SessionCoordinatorTest.kt` using `kotlinx-coroutines-test`: a connection that authorizes and then stays silent is refused at 5 s, and never appears in the connected list or consumes a roster slot (FR-017, SC-004). *(Written in `«test»/domain/control/SessionControlConnectionHandlerTest.kt`, not `SessionCoordinatorTest.kt`: `SessionCoordinator` has no suspending method and never reads a frame, so it cannot observe a timeout and the test as specified could not be written. It still asserts everything listed — refused at 5 s, absent from `connectedDevices`, no roster slot — under `runTest`, and completes in 0.07 s of wall time, proving the deadline is virtual.)*
- [X] T047 [US2] Add all five refusal cases to `«test»/gate/LoopbackJoinGateTest.kt`: wrong token → exit 3 `invalid_token`; `--protocol-version 2` → exit 3 `protocol_mismatch`; `--malformed-hello clientId` and `--malformed-hello invalid-json` → exit 3 `invalid_message`; `--silent-handshake --join-timeout 10` → exit 3 `invalid_message` within 5 s. Assert `closeCode` is 1008 and `closeReason` equals the error code in every case. *(Four cases added; the wrong-token case already landed with T037. The silent-handshake case gives the peer `--join-timeout 10`, deliberately longer than the TV's 5 s deadline: without an enforced deadline the peer would give up first and exit 6 (`no_response`), so reaching exit 3 with an error frame is itself the proof. It measures 5.6 s of real wall time against a real socket and a real clock, which is the half the virtual-time unit test T046 cannot reach.)*

### Implementation for User Story 2

- [X] T048 [P] [US2] Implement `JoinCodeMatcher` in `«main»/domain/control/JoinCodeMatcher.kt`: `normalize` uppercases and strips hyphens and surrounding whitespace; `matches` returns false for null or blank. *(Pulled forward into T036: `SessionCoordinator.authorize` cannot check a token without the matcher, and leaving it unimplemented made the loopback gate admit anyone. Implemented with `JoinCodeMatcherTest`. See spec.md Observation 19.)*
- [X] T049 [US2] Implement `HandshakeValidator` in `«main»/domain/control/HandshakeValidator.kt` with `AdmissionDecision.kt` alongside. Follow the normative order from contracts/domain-api.md: parse failure, then wrong `type`, then `protocolVersion`, then missing fields in `required`-array order, then range checks. Version precedes field presence because a version-2 phone may legitimately carry a different field set. Decode to `JsonObject` first so the reported missing field is deterministic rather than dependent on kotlinx's batching. *(`AdmissionDecision.kt` already existed from T022 and now has three cases — see spec.md Observation 19 — so nothing was created "alongside". Only two message strings are fixture-pinned (`Missing required field: X`, `Unsupported protocolVersion: N`); the wording for a wrong `type`, unparseable JSON, an unknown extra field and the two range checks is this implementation's own choice and is **not** normative. An absent `type` or `protocolVersion` deliberately falls through to the missing-field check rather than reporting `Unsupported protocolVersion: null`.)*
- [X] T050 [US2] Implement `SessionCoordinator.authorize` to reject a missing or wrong token with `invalid_token` (FR-009), and complete `admit`'s refusal branches for `protocol_mismatch`, `invalid_message` and `session_full`. *(The `invalid_token` half already landed with T036/T048 — see spec.md Observation 19. What actually remained was narrower than the task text: `admit` receives an already-parsed `Hello`, so `protocol_mismatch` and `invalid_message` are structurally unreachable inside it — those belong to the raw frame, and the real work was wiring T049's `HandshakeValidator` into `SessionControlConnectionHandler`, which had been silently dropping frames it could not decode. `admit` itself gained only the `session_full` refusal. `session_locked` is emitted nowhere in this slice. See spec.md Observation 20.)*
- [X] T051 [US2] Add capacity to `SessionRoster.admit`: return `AtCapacity` when `size == capacity` **and** the `deviceId` is previously unseen (FR-015). A known device must never be refused for capacity.
- [X] T052 [US2] Enforce the handshake deadline in `KtorControlTransport`: wrap the wait for the first frame in `withTimeoutOrNull(5.seconds)` and on expiry call `refuse("invalid_message", …)` — the `error` frame is delivered, then the close (FR-017, research.md R6). A pending connection must hold no roster slot. *(Enforced in `SessionControlConnectionHandler`, not `KtorControlTransport`: the handler is what performs the wait for the **first** frame, while the transport is a thin `ControlConnection` adapter with no notion of frame ordering. Siting it in the domain also makes it unit-testable and coverage-measured, since adapters are excluded from `--src` per research.md R8. A `FirstFrame` box distinguishes a timeout from a peer that closes cleanly inside the deadline — both are `String?` otherwise, and conflating them would refuse a peer that simply hung up.)*
- [X] T053 [US2] Implement FR-018's asymmetry in `KtorControlTransport` and the coordinator: an unexpected message type **during** the handshake is fatal and closes the connection; an unrecognised type **after** a successful handshake is logged as a warning and ignored, leaving the connection open. *(The behavioural asymmetry was already structurally correct and needed no production change: an unexpected `type` during the handshake never yields a `Hello`, so `HandshakeValidator` refuses it as fatal, while `serveAdmitted`'s drain loop already ignores post-admission frames without closing. T053 added the tests that pin both halves. **The "logged as a warning" half is NOT implemented** — the app has no logging seam and `domain.control` may not use Android types; see spec.md Observation 21.)*
...
- [X] T054 [P] [US3] Extend `«test»/domain/session/SessionRosterTest.kt` for reclaim: a known `deviceId` returns `Reclaimed` with size unchanged; reclaim succeeds at capacity; reclaim succeeds whether or not a live connection currently exists (FR-020, FR-021); and `detach` with a stale `ConnectionId` returns false and mutates nothing (FR-022).
- [X] T055 [US3] Add the reclaim cases to `«test»/gate/LoopbackJoinGateTest.kt`: ten sequential `--join-only` peers fill the roster; an 11th unseen device gets exit 3 `session_full`; one of the original ten rejoins and gets exit 0 with a **different** `connectionId` (SC-005, SC-006). Ten sequential peers suffice because a disconnected device keeps its slot in this slice — `--hold` is needed only where the live count matters. *(Caught a real defect on first run — in the test, not production: `--client-id phone-1` is 7 characters and `HandshakeValidator.MIN_CLIENT_ID_LENGTH` is 8, so peer one was refused `invalid_message` before capacity was ever reached. Ids are now `loopback-phone-N`. Worth noting the gate proved the validator genuinely runs end to end. 12 real peer runs, 7.5 s.)*
- [X] T056 [P] [US3] Add a supersession test to `«test»/domain/session/SessionCoordinatorTest.kt`: with a replacement connection admitted for a device, the superseded connection's late close must not remove the roster entry or invalidate the new `connectionId` (FR-022, acceptance scenario 4).

### Implementation for User Story 3

- [X] T057 [US3] Complete `SessionRoster.admit`'s reclaim branch and `detach`'s identity guard in `«main»/domain/session/SessionRoster.kt`: a known `deviceId` reuses its entry, refreshes `displayName`/`appVersion`/`assetPort` from the new `hello`, takes a fresh `ConnectionId`, and leaves size unchanged. `detach` no-ops unless the supplied `ConnectionId` is still the entry's active one. Also add `release` and `releaseDisconnected` — unreachable in this slice but required so capacity semantics are complete and testable now (FR-024). **Delete `SessionRosterTest.admittingAnAlreadyPresentDeviceIsNotYetImplemented` and replace it with the real reclaim assertions**: T034 left that characterization test asserting the reclaim branch throws `NotImplementedError`, so it is a deliberate tripwire that will fail the moment you implement this task. That failure is expected — do not "repair" it by keeping the branch unimplemented. Also remove `@Suppress("UnusedParameter")` from `detach`, whose `connectionId` becomes load-bearing here. **A second tripwire was added by T045**: `SessionRosterTest.capacityIsEvaluatedOnlyForPreviouslyUnseenDevices` proves a known device is never short-circuited into `AtCapacity` by asserting it still reaches the reclaim `TODO()` and throws `NotImplementedError`. Once reclaim is real, rewrite that assertion to expect `RosterAdmission.Reclaimed` with the roster size unchanged — the point of the test (capacity must not intercept a known device) still holds and must stay covered. *(Both tripwires rewritten as instructed. `release`/`releaseDisconnected` were given tests too, though T054's text did not ask for them: untested new logic would have eaten the coverage margin, and the task requires them "testable now". Reclaim precedes the capacity check, so a known device can never be refused for capacity. Scoped gate LINE 97.4%, BRANCH 94.4%; the one missed line is `clear()`'s unowned `TODO`.)*
- [X] T058 [US3] Emit `SessionEvent.Reconnected` carrying the previous `ConnectionId` from `SessionCoordinator.admit` when the roster reports `Reclaimed` (FR-019). *(`SessionEvent.Reconnected.previous` had to become nullable: FR-021 and FR-023 together guarantee the ordinary reconnect has no live connection to displace, and `ConnectionId` admits no sentinel. `null` now means nothing was displaced, non-null means a live connection was (FR-022). A returning device is always reported as `Reconnected`, never `Connected` — reporting it as a fresh connection would discard exactly what FR-019 requires to be observable. See spec.md Observation 22.)*
...
- [X] T059 Write the FR-028 validation in `«test»/presentation/join/JoinViewModelTest.kt`: each of the three start failures — bind failure, announcement failure, no usable IPv4 — surfaces the blocking notice in `JoinUiState`; acknowledging it clears the notice and returns to song selection; and the game phase is still `GamePhase.Open` afterwards, with `GamePhaseMachine` never asked for a transition (SC-008). Without this the one requirement whose entire purpose is handling failure ships unproven. *(SC-008's "never asked for a transition" is proved by positive control rather than call interception — the repo has no mocking library and `GamePhaseMachine` is final. The test asserts the phase stays `Open` across the failure paths and then drives a legal `Open -> Preparing` edge to show the phase is genuinely mutable through that same surface.)*
- [X] T060 Implement the FR-028 blocking notice: `SessionStartFailure` in `«main»/domain/session/SessionStartFailure.kt` (bind failure, announcement failure, no usable address), surfaced by `JoinViewModel` and rendered as a modal over the song-list shell, dismissible with one acknowledgement that returns to song selection. It must **not** change the game phase and must not attempt a transition — F22 has no `Open→Error` edge (SC-008). **This task adds `startFailure` and `onStartFailureAcknowledged()` to `JoinUiState`/`JoinViewModel`.** T040 deliberately omitted both even though `contracts/domain-api.md` names them, because `SessionStartFailure` is domain code this task owns; adding a field of a type that did not exist yet would have meant inventing the domain type in the presentation layer. Extend the existing state rather than introducing a parallel one.
  - **Closed by T067.** SessionStartFailure, SessionComponent.startSession returning a typed SessionStartOutcome, JoinUiState.startFailure, onStartFailureAcknowledged() and the SessionStartFailureNotice composable landed here; the last clause -- *rendered as a modal over the song-list shell* -- needed a composition root that no task owned (spec.md Observation 23), so T067 was added and now renders the notice from SessionShell's SessionStartOutcome.Failed branch. Note that JoinUiState.startFailure itself is unreachable in «main»: a JoinViewModel cannot be constructed without the coordinator and endpoint a failed start never produced, so the notice is driven from the outcome directly (spec.md Observation 24).
- [X] T061 [P] Write `JoinOverlayBoundsTest` in `«test»/presentation/join/JoinOverlayBoundsTest.kt` using `getUnclippedBoundsInRoot()` under the pinned `w960dp-h540dp-land-television-xhdpi-notouch` qualifier: every expected element present; nothing exceeding 960×540dp; the QR between 30% and 55% of the shorter viewport dimension; the join code directly beneath the QR with nothing between; and no node overlapping the QR's bounds expanded by its quiet zone (research.md R9, FR-030, FR-031).
  - **R9's quiet-zone clause was corrected during implementation.** R9 asks that no node overlap "the QR's bounds expanded by its quiet zone", but the quiet zone is not outside those bounds: `QrBitmapRenderer` passes `EncodeHintType.MARGIN = 4` and reports `moduleCount` as `BitMatrix.width`, the side length *including* the margin, so the tagged QR node already contains its own quiet zone. Expanding it again double-counts the margin and fails on the join code for exactly the reason FR-030 demands ("directly beneath"). The test uses the QR's measured bounds as-is and proves the margin exists separately, by reading the outer four rings of pixels off the render. Also fixed the Phase E gate selection — see spec.md Observation 25.
- [X] T062 Confirm no screenshot baseline is recorded by this feature — `git status` must show nothing added under a Roborazzi output directory. The gate is in verify mode and Slice 3 owns baseline creation at the corrected viewport (spec.md Assumptions). *(Confirmed: `git ls-files` matches no Roborazzi image anywhere and `git status` shows nothing added under a Roborazzi output directory. Note this is also the direct evidence for spec.md Observation 16 — there are zero committed baselines repo-wide, so `verifyRoborazziDebug` cannot pass from a clean checkout. Slice 3's problem, not this one's.)*
- [X] T063 [P] Add an FR-027 guard test in `«test»/domain/session/SessionCoordinatorTest.kt` asserting no runtime path in this slice leaves `GamePhase.Open`. *(One sweep over every runtime path this slice has — refused token, accepted token, new admission, capacity refusal, reclaim while live, stale close, real close, reclaim after a clean drop — then asserts the phase is untouched. It closes by driving a legal `Open -> Preparing` edge: without that control the test would pass just as happily against a `phase` field nothing ever writes, which would make "still Open" an artifact rather than a finding.)*
- [X] T064 Run the full feature gate from plan.md's Validation Gate section with `--rerun-tasks`, then `.\gradlew.bat :app:testDebugUnitTest --tests "*LoopbackJoinGateTest*"`. Both must pass fresh. Confirm 0 skips — FR-039 bars a skipped test from satisfying a gate. Then run the **whole** suite once more, `.\gradlew.bat :app:testDebugUnitTest --rerun-tasks`, and confirm the **54** surviving Slice 0 tests still pass unchanged (SC-009). The scoped gate selects only this feature's classes, so it cannot detect a Slice 0 regression; T005's run was taken before any behaviour changed and proves nothing about the end state. **The baseline is 54, not the 55 T005 measured**, because T025 necessarily deleted `MainActivityTest.activityStartAcquiresAndStopReleasesMulticastLock` — see the Notes below. Do not "restore" it to make the arithmetic work.
  - **The authoritative gate is plan.md's feature-level completion gate, and it passes: BRANCH 91/102 (89.2%), LINE 233/242 (96.3%).** Every FQCN in that command was checked against the tree before running it, and the per-class breakdown in `testBranchJacocoReport.xml` was read back to confirm all ten selected classes were actually measured. That check mattered: an earlier ad-hoc phase gate named `domain.session.JoinCodeMatcher` when the class is in `domain.control`, and `testBranch` reported `BUILD SUCCESSFUL` having silently dropped it rather than failing on the unknown name. The real gate measures it at BRANCH 10/10. See spec.md Observation 26.
  - **Supporting per-phase gates, all fresh with `--rerun-tasks`.** Foundation BRANCH 4/4 LINE 41/41; US1 (with `LoopbackJoinGateTest`) BRANCH 29/34 LINE 155/164; US2 BRANCH 65/86 LINE 159/168; Phase E BRANCH 7/8 LINE 33/36 — every one clear of the 0.70/0.80 thresholds. The L gate ran on its own: `LoopbackJoinGateTest` 8 tests, 0 failures, 0 skips, covering acceptance, all five refusal reasons, the roster cap, reclaim, and the handshake deadline against a real socket. Whole suite: **176 tests, 0 failures, 0 errors, 0 skipped** (FR-039). Full `:app:detekt` clean.
  - **Amended after T065's probe landed: the suite is now 177 tests with 1 deliberate skip.** The added test is `LanDiscoveryProbe`, which is opt-in via `-PlanDiscoveryProbe=true` and skips otherwise (spec.md Observation 27). FR-039 is not weakened: it bars a *gate* from being satisfied by a skipped test, and this class appears in no `--test` list in plan.md, so it can satisfy nothing. With the probe enabled the count is 177 tests and 0 skips. Every gate figure above was measured before the probe existed and is unchanged by it — the probe adds no production code and alters no `--src` selection.
  - **SC-009 holds, but the expected survivor count in this task is off by one — it is 53, not 54.** Measured directly: at `7fb507b`, the last commit before this feature changed behaviour, the thirteen Slice 0 test files carried **54** `@Test` methods; today they carry **53**, all passing. The single delta is `MainActivityTest.kt`, deleted by `614578d` with exactly one test in it. The Notes entry below had labelled its "54 Slice 0 + 18 Slice 1 = 72" tally as the state *after* T025, but 72 is what the tree holds at the deletion commit's *parent* — it is the pre-T025 count. Corrected there. T005's "expect 55" was a planning-time estimate that was never accurate: the real pre-feature total was 54, because `7fb507b` ("Absorb the 2026-05-17 fixture revision", inherited from `DH1-fixture-drift-repair` and not this feature's work) had since added one test each to `ScoringEngineFixtureTest` and `FixtureSnapshotsTest`. Nothing was restored to make the arithmetic work.
- [ ] T065 Walk quickstart.md's "Manual run on a real LAN" end to end: `:app` installed on a real Android device on the LAN, and `mock-phone --join-only --discover` run from a **different** machine on that same LAN, to prove FR-004 and SC-001 outside loopback. `--discover` is mandatory — omitting `--tv-host` is an argparse error, not a fallback to mDNS — and it deliberately ignores `--token`, so a successful join proves the TV registered the service and published the right code in its TXT record. Then verify SC-002's payload half: decode the on-screen QR with any stock camera app, confirm it carries the address the TV is actually reachable on, and dial exactly that with `--tv-host`. **SC-002's single-scan gesture is out of scope and stays unproven** — no companion phone app exists in this repository or on any branch, so no task in this slice can prove it. See spec.md Out-of-Scope Observation 14.
  - **Partly pre-verified without hardware — see spec.md Observation 27.** `LanDiscoveryProbe` (opt-in: `-PlanDiscoveryProbe=true`, documented in quickstart.md) runs the real `SessionComponent.startSession` path with the real `JmdnsSessionAnnouncer` over a developer machine's LAN interface, and the real peer discovers and joins it by mDNS alone — no `--tv-host`, no `--token`, so the address, port and join code all came from the TXT record. That closes the "did we register the service and publish the right code" half of this task in advance. Two things it cannot close, so this task stays open: the `WifiManager.MulticastLock` behaviour, which has no desktop equivalent and whose failure is silent, and the QR decode with a stock camera. One correction to this task's wording: `--discover` does **not** merely ignore `--tv-host`, it is mutually exclusive with it, so `--discover --tv-host …` is an argparse error rather than a discovery attempt. The peer's own `--help` says "ignores" and is misleading; quickstart.md already had it right.

- [X] T067 Wire the presentation layer into `«main»/MainActivity.kt`'s composition root, so the join surface is reachable in the running app. Compose `JoinOverlay` over `SongListScreen` from a `JoinViewModel` built on the started session's `SessionCoordinator` and a `ControlEndpoint(result.address, result.boundPort)`, routing `SongListScreen`'s `onJoinClick` to open it and `onDismissRequest` to close it (FR-029, FR-033). On the `SessionStartOutcome.Failed` path render `SessionStartFailureNotice` over the shell instead, acknowledgeable once to return to song selection (FR-028), which completes T060's final clause. **Added during implementation** — no task owned the composition root, so `JoinOverlay` was never composed anywhere outside its own `@Preview`s and the QR never reached the screen. T065 cannot photograph a QR that is never displayed, and no unit, screenshot or loopback gate can observe the gap. See spec.md Observation 23.
- [X] T066 [P] Update `specs/003-phone-joins/spec.md`, marking Implementation Readiness items 5–9 resolved with the commits that closed them.
...
- **The Slice 0 baseline is 54 tests from T025 onward, not 55.** T025 deletes the activity-scoped multicast lock, and `MainActivityTest` contained exactly one test — `activityStartAcquiresAndStopReleasesMulticastLock` — which reflectively read `MainActivity.multicastLock` and asserted acquire-on-start/release-on-stop. That is precisely the behaviour FR-005 and research.md R5 call wrong and T025 exists to remove, so the test asserted the defect. It was deleted rather than rewritten: post-T025 `MainActivity` holds no multicast state to assert, and `MainActivity` is not `--src` selected, so a replacement would have been decoration. FR-005 is proved instead by T036's wiring and the L gate. Counts: **54 Slice 0 before T025, 53 after** — `54 + 18 = 72` is the tally at the deletion commit's parent, i.e. *before* the removal, so the post-T025 total is 71. T064 measured both ends and confirms 53 Slice 0 tests survive, all green.

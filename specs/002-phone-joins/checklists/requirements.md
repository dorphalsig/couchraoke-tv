...
- **Four requirements are proved by gates rather than by a user-visible scenario**: FR-005 (multicast lock), FR-006 (local-network permission), FR-032 (framework containment), FR-033 (version catalogue). Each is covered by the edge cases and by SC-009 / FR-034–FR-035 rather than by an acceptance scenario, because none of them is separately observable to the host.
- **Five clarifications were resolved with the user** and are recorded in the spec's Clarifications section; no assumption was silently substituted for a decision that changed scope.

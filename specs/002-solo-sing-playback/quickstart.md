...
5. **LibVLC playback seam**
   ...
   - Capture `Prepared` duration before countdown/live playback.
   - Capture `Ready(songStartTvMs)` from first audio `Playing`, with 500ms fallback.
   - Enforce `stopAtLyricsTimeMs` and emit `Ended`.
   - Implement audio-focus request/loss behavior and LibVLC error modal.
...
Use scoped `testBranch` for each task and the final feature. Final feature validation should use selectors matching the implemented production and test classes, based on the plan’s Validation Gate, and must explicitly include F15-style session lifecycle coverage, minimal song-start clock-sync gate coverage, F22 GamePhase FSM coverage, actual Join QR renderer short/full payload sizing coverage, emulator execution, and the cumulative TV-owned end-to-end flow through the TV UI.
...
- QR encodes full WebSocket URL with token and short/full endpoint payloads render centered inside the requested square with a 4-module quiet zone.
...
- UI emits `Prepared` before countdown/live playback.
- UI emits `Ready(songStartTvMs)` and coordinator gates `ScoringEngine.setSongStart` on it.
- UI enforces `stopAtLyricsTimeMs` and returns to Song List on normal end.
- Playback error returns to Song List with blocking modal and session Open.
